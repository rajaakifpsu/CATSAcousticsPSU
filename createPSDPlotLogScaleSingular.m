%Purpose: to create a power spectral density plot for a single mic, log
%       scale on the x axis
...
function createPSDPlotLogScaleSingular(fpwelch, PSD, graphtitle, micNum, xlimits, ylimits, graphPos, newfig)
    if newfig %only creates new figure if newfig true
        figure()
%   else
%       hold on;
    end
    semilogx(fpwelch,PSD(:,micNum))
    xlabel('Frequency (Hz)')
