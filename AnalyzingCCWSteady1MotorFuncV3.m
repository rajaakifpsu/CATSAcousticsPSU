%Version 3: Does not use error or percent error. Only uses st dev. Also
%window size decreased to 4s

%function [testData,DIVINEwindows]=AnalyzingCCWSteady1MotorFuncV2(tests, testnums)
function DIVINEwindows=AnalyzingCCWSteady1MotorFuncV2(testData, tests, testnums)
    %different headers depending on how you want to use it. I'm never
    %running this in any code, just the command window. So I can change it
    %up as I please and it doesnt matter.

%%testData is the data from all the full.txts
%tests is array of strings for names of the tests' full.txt files
...
%its standard deviation and percent error to average
...
for i=1:length(tests) %loops through all the tests
    ...
    while endmeasurement<length(testData(:,1,1)) %loops through each __s window.
        lm=lm+1;
        for j=19:24 %loops through all the columns corresponding to forces and moments
            avErrors(lm+1,[1,2],i)=[(startmeasurement-1)/sampleRate,endmeasurement/sampleRate]; %stores starttime & endtime
...
DIVINEwindows(1:length(tests) , 1)=testnums';
[DIVINEwindows(:,[2,3]), ~, ~]=determineWindow(stanDevs, metricWeights);
